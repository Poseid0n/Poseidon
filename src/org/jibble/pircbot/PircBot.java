...
import irc.poseidon.Poseidon;
import irc.poseidon.ProxyObject;
...
public abstract class PircBot implements ReplyConstants {
    ...
    public final synchronized void connect(String hostname, int port, ProxyObject o) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if(o == null) {
            this.log("[SAFETY] Disconnecting due to null Proxy, could give IP away.");
            return;
        }

        Socket socket;
        try {
            socket = new Socket(o.getProxyFrom());
            InetSocketAddress dest = new InetSocketAddress(_server, _port);
            socket.connect(dest);
        } catch (Exception e) {
            this.log("Proxy failed. Removing.");
            Poseidon.proxyList.remove(Poseidon.proxyList.indexOf(o));
            return;
        }
    ...
    public final synchronized void reconnect() throws IOException, IrcException, NickAlreadyInUseException{
        ...
        connect(getServer(), getPort(), getProxy());
    }
    ...
    public final ProxyObject getProxy() {
        return _proxy;
    }
